...
pub fn edit(args: &Value, cfg: &Config) -> Result<String, String> {
    ...
    if count == 0 {
        return Err("未找到匹配文本。请先 read 该文件确认原文（注意空格与换行须逐字一致）。".into());
    }
    if count > 1 && !all {
        return Err(format!("匹配了 {count} 处。请提供更长的唯一片段，或设 all=true 全部替换。"));
    }
    let replaced = if all { text.replace(old, new) } else { text.replacen(old, new, 1) };
    std::fs::write(&p, &replaced).map_err(|e| e.to_string())?;
    Ok(format!("已替换 {path} 中 {} 处", if all { count } else { 1 }))
}
...
pub fn glob(args: &Value, cfg: &Config) -> Result<String, String> {
    ...
    for entry in ignore::WalkBuilder::new(&root).hidden(false).build().flatten() {
        ...
        if matcher.is_match(&rel_s) || matcher.is_match(p.file_name().unwrap_or_default().to_string_lossy().as_ref()) {
            let mtime = entry
                .metadata()
                .ok()
...
pub fn grep(args: &Value, cfg: &Config) -> Result<String, String> {
    ...
    let mut scan = |p: &Path| -> bool {
        ...
        let Ok(bytes) = std::fs::read(p) else { return true };
        if bytes.len() > 2_000_000 || is_binary(&bytes) {
            return true;
        }
        let text = String::from_utf8_lossy(&bytes);
        let rel = p.strip_prefix(&root).unwrap_or(p).to_string_lossy().replace('\\', "/");
        let label = if rel.is_empty() { p.to_string_lossy().to_string() } else { rel };

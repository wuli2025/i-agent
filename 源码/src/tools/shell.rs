...
fn git_bash() -> Option<String> {
    ...
    // Python — exactly the environment the rest of the run assumes. Users install Git
    // anywhere (this box: D:\Users\应用\Git), so locate it from git.exe rather than
    // guessing at Program Files: git.exe lives in <root>\cmd or <root>\bin, and bash
    // is at <root>\bin\bash.exe.
...
pub fn run(args: &Value, cfg: &Config) -> Result<String, String> {
    ...
    let timeout = args.get("timeout_s").and_then(|v| v.as_u64()).unwrap_or(120).clamp(1, 600);
    ...
    match status {
        Some(st) => {
            ...
            if code == 0 {
                Ok(if text.is_empty() { "（命令成功，无输出）".into() } else { text })
            } else {
                Ok(format!("退出码 {code}\n{text}"))
            }
        }

...
pub fn convert(args: &Value, cfg: &Config) -> Result<String, String> {
    let sources = args.get("sources").and_then(|v| v.as_array()).ok_or("缺少 sources")?;
    ...
    for (i, src) in sources.iter().enumerate() {
        let csv_path = src.get("csv").and_then(|v| v.as_str()).ok_or("source 缺少 csv")?;
        let name = src
            .get("name")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string())
...
mod tests {
    ...
    fn write_real_file() {
        ...
        std::fs::write(dir.join("t.csv"), "品名,单价,数量\n苹果,3.5,10\n梨,2,\"5\"\n").unwrap();

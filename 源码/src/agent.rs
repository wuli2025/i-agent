...
impl Agent {
    ...
    fn turn_loop(&mut self, on_text: &dyn Fn(&str)) -> Result<String, String> {
        let max_turns = if self.depth == 0 { self.cfg.max_turns } else { self.cfg.max_turns / 2 };
        ...
        for turn in 0..max_turns {
            ...
            if tool_calls.is_empty() {
                ...
                if !pending.is_empty() && gate_nudges < 2 {
                    gate_nudges += 1;
                    self.log(&format!("▸ 交付门禁：{} 尚未通过浏览器验证", pending.join(", ")));
                    self.messages.push(Msg::text(
                        "user",
                        &format!(
    ...
    fn exec_tools(&mut self, tool_calls: &[ToolCall]) -> Vec<(String, String)> {
        // 多个 task 并行；其余顺序执行
        let tasks: Vec<&ToolCall> =
            tool_calls.iter().filter(|t| t.function.name == "task").collect();

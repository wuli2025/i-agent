...
-> Vec<(&'static str, &'static str, &'static str, &'static str, &'static [&'static str])> {
    // name, base, default model, protocol, key envs（按序取第一个非空）
    vec![
        ("deepseek", "https://api.deepseek.com/anthropic", "deepseek-chat", "anthropic", &[
            "DEEPSEEK_API_KEY",
        ]),
        ("kimi", "https://api.kimi.com/coding", "k3", "anthropic", &["KIMI_API_KEY"]),
        // 月之暗面开放平台的老密钥与 kimi.com 不通用，单独保留一条 OpenAI 兼容泳道
        ("moonshot", "https://api.moonshot.cn/v1", "kimi-k2-0905-preview", "openai", &[
            "MOONSHOT_API_KEY",
        ]),
        ("glm", "https://open.bigmodel.cn/api/anthropic", "glm-4.6", "anthropic", &[
            "ZHIPU_API_KEY",
            "GLM_API_KEY",
        ]),
        (
        ...
        ("minimax", "https://api.minimaxi.com/anthropic", "MiniMax-M3", "anthropic", &[
            "MINIMAX_API_KEY",
        ]),
    ]
}
...
fn anthropic_env_provider() -> Option<Provider> {
    ...
    let base = env_nonempty("ANTHROPIC_BASE_URL")
...
fn builtin_image_providers() -> Vec<(&'static str, &'static str, &'static str, &'static str)> {
    vec![
        ("zhipu", "https://open.bigmodel.cn/api/paas/v4", "cogview-4", "ZHIPU_API_KEY"),
        (
            "siliconflow",
            "https://api.siliconflow.cn/v1",
            "Kwai-Kolors/Kolors",
        ...
        ("minimax", "https://api.minimaxi.com/v1", "image-01", "MINIMAX_API_KEY"),
    ]
}
...
impl Config {
    pub fn load(
        ...
        if let (Some(key), Some(base)) =
            (env_nonempty("I_AGENT_API_KEY"), env_nonempty("I_AGENT_BASE_URL"))
        {
            let proto = detect_protocol(&env_nonempty("I_AGENT_PROTOCOL").unwrap_or_default(), &base);
        ...
        for (name, base, model, protocol, key_envs) in builtin_providers() {
            ...
            let key = key_envs.iter().find_map(|k| env_nonempty(k)).unwrap_or_default();
        ...
        let usable_explicit: Vec<Provider> =
            explicit.iter().filter(|p| !p.key.is_empty()).cloned().collect();
        let usable_builtin: Vec<Provider> =
            builtin.iter().filter(|p| !p.key.is_empty()).cloned().collect();
        ...
        let mut ordered: Vec<Provider> = match &provider_sel {
            Some(sel) => {
                ...
                let mut v: Vec<Provider> =
                    all.iter().filter(|p| &p.name == sel).cloned().collect();
                if v.is_empty() {
                    return Err(format!(

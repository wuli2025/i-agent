...
pub fn run(args: &Value, cfg: &Config) -> Result<String, String> {
    ...
    let auto = if lower.ends_with(".html") || lower.ends_with(".htm") {
        "html"
    } else {
        "json"
    };
...
fn check_json(path: &str, text: &str) -> Result<String, String> {
    ...
    if let (Some(scenes), Some(endings)) = (
        v.get("scenes").and_then(|s| s.as_array()),
        v.get("endings").and_then(|s| s.as_array()),
    ) {
        let mut shape: Vec<String> = Vec::new();
        for field in ["stats", "affinity", "flags"] {
            if let Some(fv) = v.get(field) {
                match fv.as_array() {
                    None => shape.push(format!(
                        "{field} 必须是数组（形如 [{{\"key\":...,\"name\":...}}]），不能是对象"
                    )),
                    Some(arr) => {
                        if arr
                            .iter()
                            .any(|e| e.get("key").and_then(|k| k.as_str()).is_none())
                        {
                        ...
                        if field == "stats"
                            && arr
                                .iter()
                                .any(|e| e.get("init").is_none() || e.get("max").is_none())
                        {
                            shape.push(
                                "stats 每项必须有 init 与 max 字段（不是 initial/min）".into(),
                            );
                        }
                    }
                }
        ...
        if let Some(start) = v.get("start").and_then(|s| s.as_str()) {
            if !scenes
                .iter()
                .any(|s| s.get("id").and_then(|i| i.as_str()) == Some(start))
            {
        ...
        let ids: std::collections::HashSet<&str> = scenes
            .iter()
